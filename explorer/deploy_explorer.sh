...
function config(){
	...
	network_config_file=$(pwd)/$fabricBlockchainNetworkName/$CONFIG_JSON
	#configure explorer to connect to specific Blockchain network using given crypto materials
	network_crypto_base_path=$(pwd)/$fabricBlockchainNetworkName/crypto
...
function copyCerts() {
	rm -rf  ./dev/crypto/peerOrganizations
	rm -rf  ./dev/crypto/ordererOrganizations

	mkdir -p ./dev/crypto/peerOrganizations/
	cp -r ../network-setup/org1.example.com/crypto-config/ordererOrganizations ./dev/crypto/
	cp -r ../network-setup/org1.example.com/crypto-config/peerOrganizations/org1.example.com ./dev/crypto/peerOrganizations/
	cp -r ../network-setup/org2.example.com/crypto-config/peerOrganizations/org2.example.com ./dev/crypto/peerOrganizations/
}
